...
/**
  ...
  */
class DumpImagesPretty extends CliTool {

  import DumpImagesPretty._
  ...
  def init(): Unit = {
    ...
    conf.setAppName("DumpImagesPretty")
  ...
  override def run(): Unit = {
    ...
    */
    ...
    val cdrRDD = filteredRDD.map({case(text, content) =>
      (CommonUtil.reverseUrl(text), content.getContent)})
...
object DumpImagesPretty extends Loggable with Serializable {
  ...
  def saveImage(id: String, content: Array[Byte]): Unit = {
    ...
    val dir = id.substring(0, id.lastIndexOf(File.separator))
    //val name = id.substring(id.lastIndexOf(File.separator) + 1)
    CommonUtil.makeSafeDir(dir)
